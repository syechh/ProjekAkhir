...
if(isset($_POST['tambah_item'])){
    ...
    $query = mysqli_query($con, "SELECT nama_barang, harga_beli, supplier FROM barang WHERE id='$id_barang'");
    ...
    if($data){
        ...
        $_SESSION['keranjang_pembelian'][] = $item;
    }
}
...
if(isset($_GET['hapus_index'])){
    ...
    unset($_SESSION['keranjang_pembelian'][$index]);
    $_SESSION['keranjang_pembelian'] = array_values($_SESSION['keranjang_pembelian']);
...
if(isset($_POST['simpan_transaksi'])){
    ...
    if(!empty($_SESSION['keranjang_pembelian'])){
        ...
        foreach($_SESSION['keranjang_pembelian'] as $cart){
            $id_brg = $cart['id'];
            $qty = $cart['qty'];
...
if(!empty($_SESSION['keranjang_pembelian'])){
    $supplier_otomatis = $_SESSION['keranjang_pembelian'][0]['supplier'];
}
?>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="id_barang" class="form-control" required>
                                ...
                                while($b = mysqli_fetch_array($ambil)){
                                    echo "<option value='".$b['id']."'>".$b['nama_barang']." (Supl: ".$b['supplier'].")</option>";
                                }
                                ?>
                            </select>
                        </div>
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                if(!empty($_SESSION['keranjang_pembelian'])){
                                    $no = 1;
                                    $grand_total = 0;
                                    foreach($_SESSION['keranjang_pembelian'] as $key => $isi){
                                        $grand_total += $isi['subtotal'];
                                ?>
                                <tr>
                                    <td><?= $no++; ?></td>
                                    <td><?= $isi['nama']; ?></td>
                                    <td><?= $isi['supplier']; ?></td> <td><?= $isi['qty']; ?></td>
                                    <td>Rp <?= number_format($isi['harga']); ?></td>
                                    <td>Rp <?= number_format($isi['subtotal']); ?></td>
                                    <td>
                                        <a href="pembelian.php?hapus_index=<?= $key; ?>" class="btn btn-danger btn-sm">Hapus</a>
                                    </td>
                                </tr>
                                <?php } ?>
                                <tr class="fw-bold">
